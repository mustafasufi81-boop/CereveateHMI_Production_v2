...
Write-Host "PHASE 2 LOAD & STRESS TESTS" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
try {
    ...
    if ($successCount -ge 95 -and $healthCheck.overallStatus -eq "Healthy") {
        ...
        $results += @{ Test = "11.1"; Status = "FAIL"; Details = "$successCount/100 succeeded, health: $($healthCheck.status)" }
    }
} catch {
...
try {
    ...
    if ($successCount -ge 950 -and $healthCheck.status -eq "Healthy") {
        ...
        Write-Host "⚠️ PARTIAL - $successCount/$totalRequests succeeded, health: $($healthCheck.status)" -ForegroundColor Yellow
        $results += @{ Test = "11.2"; Status = "PARTIAL"; Details = "$successCount/$totalRequests @ ${actualRps} req/s" }
    }
} catch {
    Write-Host "❌ FAIL - $($_.Exception.Message)" -ForegroundColor Red
...
try {
    ...
    $beforeStatus = Invoke-RestMethod -Uri "$BaseUrl/api/plc/connections" -Method Get
    $plc = $beforeStatus | Where-Object { $_.plcId -eq "Rockwel_PLC_001" }
    
    if ($plc) {
        Write-Host "Current state: $($plc.state), consecutiveFailures: $($plc.consecutiveFailures)" -ForegroundColor Gray
        
        # Monitor for 30 seconds to check stability
        Start-Sleep -Seconds 30
        
        $afterStatus = Invoke-RestMethod -Uri "$BaseUrl/api/plc/connections" -Method Get
        $plcAfter = $afterStatus | Where-Object { $_.plcId -eq "Rockwel_PLC_001" }
        
        if ($plcAfter.state -eq "Running" -and $plcAfter.consecutiveFailures -eq 0) {
            Write-Host "✅ PASS - PLC in Running state with 0 consecutive failures" -ForegroundColor Green
            $results += @{ Test = "7.1"; Status = "PASS"; Details = "State: $($plcAfter.state), CF: $($plcAfter.consecutiveFailures)" }
        } else {
            Write-Host "⚠️ PARTIAL - State: $($plcAfter.state), CF: $($plcAfter.consecutiveFailures)" -ForegroundColor Yellow
            $results += @{ Test = "7.1"; Status = "PARTIAL"; Details = "State: $($plcAfter.state), CF: $($plcAfter.consecutiveFailures)" }
        }
    } else {
...
try {
    $status = Invoke-RestMethod -Uri "$BaseUrl/api/plc/connections" -Method Get
    $plc = $status | Where-Object { $_.plcId -eq "Rockwel_PLC_001" }
    
    if ($plc) {
        $hasRecoveryMetrics = $plc.PSObject.Properties.Name -contains "consecutiveFailures" -and 
                              $plc.PSObject.Properties.Name -contains "lastSuccessTime" -and
                              $plc.PSObject.Properties.Name -contains "lastFailureTime"
        
        $isHealthy = $plc.state -eq "Running" -and $plc.consecutiveFailures -lt 5
        
        if ($hasRecoveryMetrics -and $isHealthy) {
            Write-Host "✅ PASS - Recovery metrics present, system healthy" -ForegroundColor Green
            $results += @{ Test = "9.2"; Status = "PASS"; Details = "CF: $($plc.consecutiveFailures), State: $($plc.state)" }
        } else {
...
$results | ForEach-Object {
    [PSCustomObject]$_
} | Export-Csv -Path "phase2_test_results.csv" -NoTypeInformation

Write-Host "`nResults exported to: phase2_test_results.csv" -ForegroundColor Cyan
