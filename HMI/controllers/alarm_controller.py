    ...
    try:
        ...
        check_query = """
            SELECT aa.alarm_state, aa.alarm_key,
                   ...
                   aa.raised_value, aa.setpoint_value
            FROM historian_raw.alarm_active aa
            WHERE aa.current_event_id = %s
        """
        ...
        # C# ClearAsync(forceAck=true) handles ACTIVE_UNACK → ACK → CLEAR atomically
        # inside its own per-key SemaphoreSlim(1,1). DO NOT do a separate ACK call here —
        # that creates a race window between the ACK and CLEAR proxies.
        # Allowed: ACTIVE_UNACK (C# will auto-ACK), ACTIVE_ACK (direct clear)
        # Rejected: RTN_UNACK (must ACK separately per ISA-18.2), CLEARED, unknown
        if alarm_state == 'ACTIVE_UNACK':
            logger.info(f"Alarm {alarm_id} is ACTIVE_UNACK — C# will auto-ACK via forceAck=true atomically")
        ...
        try:
            ...
            if not has_raised:
                ...
                if row:
                    ...
                    audit_records.sort(
                        key=lambda r: (_audit_action_rank(r.get('action_type')), (r.get('action_timestamp') or ''))
                    )
                    logger.info("Backfilled missing RAISED audit record for alarm %s", alarm_id)
            
            logger.info(f"Retrieved {len(audit_records)} audit records for alarm {alarm_id}")
    ...
    try:
        ...
        for row in rows:
            ...
            records.append({
                ...
                'severity':           r['severity'],
                'acknowledged_by':    r['ack_operator'] or r['acknowledged_by'],
                'acknowledged_at':    _fmt(r['ack_timestamp'] or r['acknowledged_at']),
                'ack_notes':          r['ack_notes'],
                'cleared_by':         r['clear_operator'] or r['cleared_by'],
...
def suppress_alarm(alarm_id):
    ...
    try:
        ...
        cur.execute(
            "SELECT alarm_key, level, tag_id, priority, alarm_state FROM historian_raw.alarm_active WHERE current_event_id = %s",
            (alarm_id,)
        )
        row = cur.fetchone()
        cur.close()
        ...
        if isinstance(row, dict):
            ...
            alarm_key, alarm_level, tag_id, priority, actual_state = row
        ...
        cur.execute("""
            INSERT INTO historian_raw.alarm_audit_trail
                (event_id, tag_id, event_type, action_type, action_timestamp,
                 ...
                 action_reason, action_notes, client_ip, metadata)
            VALUES (%s, %s, 'ALARM', 'SUPPRESSED', NOW(),
                    %s, %s, 'SUPPRESSED', %s,
                    %s, %s, %s, %s::jsonb)
        """, (
            alarm_id, tag_id, username, actual_state, priority,
            reason, notes, request.remote_addr, metadata
        ))
        db_service.connection.commit()
        cur.close()
...
def unsuppress_alarm(alarm_id):
    ...
    try:
        ...
        cur = db_service.connection.cursor()
        cur.execute(
            "SELECT alarm_key, level, tag_id, priority FROM historian_raw.alarm_active WHERE current_event_id = %s",
            (alarm_id,)
        )
        row = cur.fetchone()
        cur.close()
        ...
        if isinstance(row, dict):
            ...
            alarm_key, alarm_level, tag_id, priority = row
        ...
        cur.execute("""
            INSERT INTO historian_raw.alarm_audit_trail
                (event_id, tag_id, event_type, action_type, action_timestamp,
                 performed_by, previous_state, new_state, alarm_priority,
                 action_reason, client_ip, metadata)
            VALUES (%s, %s, 'ALARM', 'UNSUPPRESSED', NOW(),
                    %s, 'SUPPRESSED', 'ACTIVE_UNACK', %s,
                    'Operator lifted suppression', %s, %s::jsonb)
        """, (alarm_id, tag_id, username, priority, request.remote_addr, metadata))
        db_service.connection.commit()
        cur.close()
