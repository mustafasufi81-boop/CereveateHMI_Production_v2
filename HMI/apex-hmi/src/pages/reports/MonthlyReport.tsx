...
import { downloadMonthlyReportXlsx, downloadMonthlyReportXlsxFiltered, fetchMonthlyReport } from "@/services/reportApi";
...
const MonthlyReport = () => {
  ...
  const uniqueSources = useMemo(() => {
    ...
    (areasQuery.data || []).forEach((x) => {
      if (x.server_progid && x.server_progid !== "Unknown") set.add(x.server_progid);
    });
    return Array.from(set).sort();
  }, [areasQuery.data]);

  // CASCADE Step 2 — plants filtered by source
  ...
  // CASCADE Step 3 — areas filtered by source + plants
  ...
  // Reset plants+areas when source changes
  useEffect(() => {
    setSelectedPlants([]);
    setSelectedAreas([]);
  }, [selectedSource]);

  // Auto-select all valid areas when plants change
  useEffect(() => {
    setSelectedAreas((prev) => {
      const valid = prev.filter((a) => areasForPlants.includes(a));
      if (selectedPlants.length > 0 && valid.length === 0 && areasForPlants.length > 0)
        return areasForPlants;
      return valid;
    });
  }, [areasForPlants]);
  ...
  const selectAllPlants = () => setSelectedPlants(uniquePlants);
  ...
  const allTags = useMemo(
    () => rows.map((row) => ({ id: row.tag_id, label: row.display_label || row.tag_id })),
    [rows]
  );
  ...
  const filteredTagsForDropdown = useMemo(() => {
    ...
    return allTags.filter(
      (t) => t.id.toLowerCase().includes(s) || t.label.toLowerCase().includes(s)
    );
  ...
  // Persist selection to context
  useEffect(() => {
    updateSelection({ date: fromDate, selectedPlants, selectedAreas });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fromDate, selectedPlants, selectedAreas]);

  // Sync table scroll width
  useEffect(() => {
  ...
  const onGenerate = () => {
    ...
    if (!fromDate || !toDate || selectedPlants.length === 0 || selectedAreas.length === 0) {
      setErrorMsg("Select from date, to date, at least one plant and at least one area.");
      return;
    }
    const days = inclusiveDays(fromDate, toDate);
    if (days < 1 || days > 31) {
      setErrorMsg("Date range must be between 1 and 31 days (inclusive).");
      return;
    }
    setErrorMsg("");
    setCurrentPage(1);
  ...
  const onDownload = async () => {
    ...
    try {
      await downloadMonthlyReportXlsx(
        fromDate,
        toDate,
        selectedPlants.join(","),
        selectedAreas.join(","),
  ...
  const onDownloadFiltered = async () => {
    ...
    const tagsToDownload = filteredRows.map((r: any) => r.tag_id);
    ...
    try {
      await downloadMonthlyReportXlsxFiltered(
        fromDate,
        toDate,
        selectedPlants.join(","),
        selectedAreas.join(","),
      ...
      <div className="max-w-[1800px] mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between bg-white rounded-lg shadow-md p-4 border border-gray-200">
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/"
              className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors shadow-md"
            >
              ← Back to HMI
            </Link>
            <UserHeader />
        ...
        <div className="bg-blue-50 border-l-4 border-blue-600 p-3 rounded-r-lg">
          <p className="text-sm font-semibold text-blue-900">📈 Date-range report view (up to 31 days)</p>
        </div>

        {/* Filter controls */}
        <div className="grid grid-cols-1 md:grid-cols-7 gap-4 bg-white p-6 rounded-lg shadow-lg border-2 border-gray-200">
          {/* From Date */}
          <div>
            <label className="text-xs font-bold text-gray-700 block mb-2">📅 From Date</label>
            <input
              type="date"
              value={fromDate}
              onChange={(e) => { setFromDate(e.target.value); setReportRequest(null); }}
          ...
          {/* To Date */}
          <div>
            <label className="text-xs font-bold text-gray-700 block mb-2">📅 To Date</label>
            <input
              type="date"
              value={toDate}
              onChange={(e) => { setToDate(e.target.value); setReportRequest(null); }}
              className="w-full bg-white border-2 border-gray-300 rounded-lg px-3 py-2 text-gray-800 font-semibold focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all"
            />
          </div>
          {/* Source */}
              ...
              {showPlantDropdown && uniquePlants.length > 0 && (
                <div className="absolute top-full left-0 mt-2 w-full max-h-80 overflow-hidden bg-white rounded-lg shadow-2xl border-2 border-blue-500 z-50 flex flex-col">
                  <div className="sticky top-0 bg-blue-50 p-3 border-b-2 border-blue-300 flex items-center justify-between gap-2">
                    <button onClick={selectAllPlants} className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-bold transition-colors">Select All</button>
                    <button onClick={clearAllPlants} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-bold transition-colors">Clear All</button>
                    <button onClick={() => setShowPlantDropdown(false)} className="px-3 py-1 bg-gray-600 hover:bg-gray-700 text-white rounded text-xs font-bold transition-colors">Close</button>
                  </div>
                  <div className="overflow-y-auto max-h-60 p-2">
                    {uniquePlants.map((plant) => (
                      <label key={plant} className="flex items-center gap-2 p-2 hover:bg-blue-50 rounded cursor-pointer transition-colors">
                        <input
                          type="checkbox"
          ...
          <div>
            ...
            <div className="relative">
              ...
              {showAreaDropdown && areasForPlants.length > 0 && (
                <div className="absolute top-full left-0 mt-2 w-full max-h-80 overflow-hidden bg-white rounded-lg shadow-2xl border-2 border-blue-500 z-50 flex flex-col">
                  <div className="sticky top-0 bg-blue-50 p-3 border-b-2 border-blue-300 flex items-center justify-between gap-2">
                    <button onClick={selectAllAreas} className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-bold transition-colors">Select All</button>
                    <button onClick={clearAllAreas} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-bold transition-colors">Clear All</button>
                    <button onClick={() => setShowAreaDropdown(false)} className="px-3 py-1 bg-gray-600 hover:bg-gray-700 text-white rounded text-xs font-bold transition-colors">Close</button>
                  </div>
                  <div className="overflow-y-auto flex-1 p-2">
                    {areasForPlants.map((area) => (
                      <label key={area} className="flex items-center gap-2 p-2 hover:bg-blue-50 rounded cursor-pointer text-gray-800">
                        <input
                          type="checkbox"
          ...
          <div className="flex items-end">
            <button
              ...
              disabled={!fromDate || !toDate || selectedPlants.length === 0 || selectedAreas.length === 0 || reportQuery.isFetching}
            >
          ...
          <div className="flex items-end">
            <button
              ...
              disabled={!fromDate || !toDate || selectedPlants.length === 0 || selectedAreas.length === 0 || isDownloading || reportQuery.isFetching}
            >
        ...
        {errorMsg && (
          <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-r-lg shadow-md font-semibold">
            ⚠️ {errorMsg}
          </div>
        )}
        {reportQuery.isLoading && (
          <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 rounded-r-lg shadow-md font-semibold">
            ⏳ Loading report...
          </div>
        )}
        {reportQuery.isError && (
        ...
        {reportQuery.data && (
          <div className="bg-white border-2 border-gray-300 rounded-lg overflow-hidden shadow-2xl">
            ...
            {/* Professional Header — blue gradient with logo */}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right text-white">
                  ...
                  <div className="font-bold text-xl">
                    {reportQuery.data.meta.from_date} — {reportQuery.data.meta.to_date}
                  </div>
                </div>
              </div>
            ...
            {/* Tag Search + Tag Filter dropdown (outside overflow-x-auto so it's never clipped) */}
            <div className="px-6 py-4 bg-gray-50 border-b-2 border-gray-200">
              <div className="flex items-center gap-3 flex-wrap">
                <label className="text-sm font-semibold text-gray-700 whitespace-nowrap">🔍 Search Tags:</label>
                <input
                  type="text"
            ...
            <div className="px-6 py-4 bg-gray-50 border-b-2 border-gray-200 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="font-bold text-gray-800 bg-white px-4 py-2 rounded-lg border-2 border-gray-300">
                  Page {totalPages ? currentPage : 0} / {totalPages}
                </span>
                <button
            ...
            <div className="px-4 py-2 border-b border-slate-700 flex items-center gap-3 bg-slate-800/60">
              ...
              <button onClick={scrollDataLeft} className="px-2 py-1 rounded bg-slate-700 hover:bg-slate-600 text-xs" aria-label="Scroll left">←</button>
              <button onClick={scrollDataRight} className="px-2 py-1 rounded bg-slate-700 hover:bg-slate-600 text-xs" aria-label="Scroll right">→</button>
              ...
              <table ref={tableRef} className="min-w-full text-sm">
                <thead className="bg-gradient-to-r from-blue-900 to-blue-800 sticky top-0 shadow-lg">
                  <tr>
                    ...
                    <th className="p-3 border-2 border-blue-700 text-white font-bold">
                      Tag Name
                      {selectedTags.length > 0 && (
                        <span className="ml-2 bg-amber-400 text-blue-900 text-xs font-bold px-2 py-0.5 rounded-full">
                          {selectedTags.length}
                        </span>
                      )}
                    </th>
                    ...
                    {reportQuery.data.columns.map((col) => (
                      <th key={col} className="p-3 border-2 border-blue-700 whitespace-nowrap text-white font-bold">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredRows.map((row, rowIdx) => (
                    <tr
                      key={`${row.s_no}-${row.tag_id}`}
                      className={`${rowIdx % 2 === 0 ? "bg-blue-50" : "bg-white"} hover:bg-amber-50 transition-colors`}
                    >
                      ...
                      <td className="p-3 border border-gray-300 text-gray-800">
                        {row.description || row.display_label || row.tag_id}
                      </td>
                      ...
                      {row.hourly.map((v, idx) => (
                        <td key={idx} className="p-3 border border-gray-300 text-right text-gray-800 font-medium">
                          {v ?? "-"}
                        </td>
                      ))}
                    </tr>
