...
export function TagValuePanel({ selectedNodeId, tags, componentName }: TagValuePanelProps) {
  ...
  const handleTagToggle = (tagId: string) => {
    ...
    if (newSelected.has(tagId)) {
      ...
      if (newSelected.size >= MAX_SELECTED_TAGS) {
        console.warn(`[TagValuePanel] Maximum ${MAX_SELECTED_TAGS} tags can be selected`);
        return;
      }
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="border-b border-sidebar-border px-4 py-3">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {tags.length} tags available • {selectedTags.size}/{MAX_SELECTED_TAGS} selected
        </p>
      </div>
