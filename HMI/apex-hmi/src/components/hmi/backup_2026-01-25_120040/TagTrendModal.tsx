...
// Generate mock historical data for individual tag
const generateTagData = (hours: number, currentValue: number, unit: string) => {
  const data = [];
  const now = new Date();
  const pointsPerHour = hours <= 4 ? 12 : hours <= 8 ? 6 : hours <= 24 ? 4 : 1;
  const totalPoints = hours * pointsPerHour;
  const baseValue = currentValue;
  const variation = unit === "°C" ? 5 : unit === "RPM" ? 50 : unit === "mm/s" ? 2 : 1;

  for (let i = totalPoints; i >= 0; i--) {
    const time = new Date(now.getTime() - (i * (60 / pointsPerHour) * 60 * 1000));
    const value = baseValue + Math.sin(i / 10) * variation + (Math.random() - 0.5) * variation;
    ...
    data.push({
      ...
      min: Math.round((value - Math.random() * variation) * 100) / 100,
      max: Math.round((value + Math.random() * variation) * 100) / 100,
    });
  }
  return data;
};
...
export const TagTrendModal = ({ 
  ...
  const data = generateTagData(selectedRange, currentValue, unit);
